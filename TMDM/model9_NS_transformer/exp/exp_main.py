...
class Exp_Main(Exp_Basic):
    ...
    def _forward_base(self, batch_x, batch_x_mark, dec_inp, batch_y_mark):
        """
        ns_Transformer.forward() 需要返回:
            y_base, dec_out, KL_loss, z_sample, enc_feat

        这里我们只使用 y_base，并强制截断到 pred_len。
        """
    ...
    def vali(self, vali_data, vali_loader, criterion):
        ...
        with torch.no_grad():
            for i, (batch_x, batch_y, batch_x_mark, batch_y_mark) in enumerate(vali_loader):
                ...
                output = self.model(batch_x, batch_x_mark, r0, r_t_batch, r_prior, t)
    ...
    def train(self, setting):
        ...
        if os.path.exists(base_ckpt):
            ...
            self.cond_pred_model.load_state_dict(
                torch.load(base_ckpt, map_location=self.device), strict=False
            )
    ...
    def test(self, setting, test=0):
            ...
            for i, (batch_x, batch_y, batch_x_mark, batch_y_mark) in enumerate(test_loader):
                ...
                for _ in range(self.model.diffusion_config.testing.n_z_samples_depart):
                    ...
                    gen_r = r_tile_seq[-1].reshape(
                        ...
                        int(self.model.diffusion_config.testing.n_z_samples /
                            self.model.diffusion_config.testing.n_z_samples_depart),
                        self.args.pred_len,
                        self.args.c_out
                    ).cpu().numpy()
        ...
        print('NT metrc: mse:{:.4f}, mae:{:.4f} , rmse:{:.4f}, mape:{:.4f}, mspe:{:.4f}'.format(
        ...
        print('CARD metrc: QICE:{:.4f}%, PICP:{:.4f}%'.format(
            qice_coverage_ratio * 100, coverage * 100
        ))
